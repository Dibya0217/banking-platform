...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiResponse<Void>> handleWebClientError(WebClientResponseException ex) {
        log.warn("Account service returned error: status={}", ex.getStatusCode());
        return ResponseEntity.status(HttpStatus.BAD_GATEWAY)
                .body(ApiResponse.error("ACCOUNT_SERVICE_ERROR", "Account service request failed: " + ex.getMessage()));
    }
